...
void single_step(map_cell* tempdata, char operator_cmd)
{
	...
	case 'A':
	{
				...
				tempdata->x = tempdata->x/100+tempdata->x%100*100;
				tempdata->y = tempdata->y / 100 + tempdata->y % 100 * 100;
				break;
	}
	case 'B':
	{
				tempdata->x = tempdata->x / 1000 + tempdata->x % 1000 * 10;
				tempdata->y = tempdata->y / 1000 + tempdata->y % 1000 * 10;
				break;
	}
	case 'C':
	{
				int temp = tempdata->x;
				tempdata->x = tempdata->x % 10 + tempdata->x / 1000 * 1000 + tempdata->x % 100 / 10 * 100 + tempdata->y % 100 / 10 * 10;
				tempdata->y = tempdata->y % 10 + tempdata->y / 1000 * 1000 + temp % 1000 / 100 * 100 + tempdata->y % 1000 / 100 * 10;
				break;
	}
	default:
		break;
...
string find_route(int step_max)
{
	...
	map_cell tempdata(1234, 5678, true, -1, -1);
	map_cell tempdata2;
	if (destiny_x == 1234 && destiny_y ==5678)
	{
		return "0";
	}
